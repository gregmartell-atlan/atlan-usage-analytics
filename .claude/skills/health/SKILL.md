...
1. **Domain** (required): "Which customer domain would you like to check? (e.g., acme.atlan.com)"
   - If they're unsure, offer to run `sql/05_customer_health/domain_summary_snapshot.sql` first to show all domains.
...
### Quick mode:
1. Read `sql/05_customer_health/customer_health_scorecard.sql`
2. Replace `{{DATABASE}}` and `{{SCHEMA}}` with values from CLAUDE.md Configuration
3. Replace `{{START_DATE}}` with computed date (single-quoted: `'YYYY-MM-DD'`)
4. Execute via the Snowflake MCP tool (see `SNOWFLAKE_MCP_TOOL` in CLAUDE.md Configuration)
5. Filter results to show only the requested domain's row

### Full mode (run sequentially):
For each SQL file below: replace `{{DATABASE}}` and `{{SCHEMA}}` with values from CLAUDE.md Configuration, then substitute other parameters.
Execute via the Snowflake MCP tool (see `SNOWFLAKE_MCP_TOOL` in CLAUDE.md Configuration).

1. **Health Score**: `sql/05_customer_health/customer_health_scorecard.sql` (param: START_DATE)
   - Filter to requested domain from results
2. **License Utilization**: `sql/05_customer_health/license_utilization.sql` (params: START_DATE, DOMAIN)
3. **Role Distribution**: `sql/05_customer_health/role_distribution.sql` (param: DOMAIN)
4. **Risk Alerts**: `sql/06_cs_review/trending_alert.sql` (param: START_DATE)
   - Filter to requested domain from results
